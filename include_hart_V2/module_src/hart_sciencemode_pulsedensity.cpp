 ...
 *
 * Special stimulator block with the ability of unregular stimulation pulses.
 * There is a new input port #4 of size 1, that is a mute signal.
 * if mute == 1 no stimulation signal is send to the stimulator.
 */
...
#include "getstr.h"
    ...
    void rt_sciencemode_pulsedensity(scicos_block *block,int flag);
...
int init_sciencemode_pulsedensity(scicos_block *block){
...
void inout_sciencemode_pulsedensity(scicos_block *block){
 ...
 struct  oScience* comdev = (struct oScience *) (*block->work);
  ...
  double *Mute=GetRealInPortPtrs(block,4); // Added CK
  ...
  double sum_pw=0;
   ...
   if (Mute > 0) // Added CK: If the blocks mute input is 1 then do not send stimulation signal to the stimulator
     return;
...
void end_sciencemode_pulsedensity(scicos_block *block){
  struct  oScience* comdev = (struct oScience *) (*block->work);
  int error;
...
void rt_sciencemode_pulsedensity(scicos_block *block,int flag)
{
  if ((flag==2)&&(block->nevprt==1)){          /* stimulate on state update */
    inout_sciencemode_pulsedensity(block);
  }
  else if (flag==5){     /* termination */
    end_sciencemode_pulsedensity(block);
  }
  else if (flag ==4){    /* initialisation */
    init_sciencemode_pulsedensity(block);
  }
}
